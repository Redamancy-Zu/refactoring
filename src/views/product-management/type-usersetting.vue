<template>
  <div>
    ...
    <el-dialog
      ...
      <el-scrollbar>
        <div class="tabMenu">
          <span><i class="typeFunction">*</i> 功能类型</span><span @click="selectType(index)" :key="index" :class="activeName == index ? 'active' : ''"  v-for="(item,index) in tablist">{{item}}</span>
          <el-popover
            class="tabPopover"
            placement="top"
            width="300"
          ...
          <ul>
            ...
            <li v-if="activeName == 0">
              <el-form :model="addUserSetting" label-width="90px" ref="addUserSetting" :rules="addUserSettingRule" @submit.native.prevent>
                ...
                <el-form-item label-width="90px" label="筛选条件" class="formItemInput" :class="!addUserSetting.openFilter?'formItemLabelColor':''" style="margin-top:15px;">
                	<el-popover
                		class="iconSpan"
								    placement="top-start"
								    width="339"
            ...
            <li v-else>
              <el-form :model="addEventForm" label-width="90px" ref="addEventForm" :rules="addEventFormRule">
                ...
                <el-form-item label="筛选条件" class="formItemInput" :class="!addEventForm.openFilter?'formItemLabelColor':''" style="margin-top:15px;">
                	<el-popover
                		class="iconSpan"
								    placement="top-start"
								    width="339"
    ...
    <el-dialog
      ...
      top="40vh">
        ...
        <div style="text-align:right;" slot="footer" class="dialog-footer">
          <el-button class="closeToggle" @click="toggleTipVisible = false">取 消</el-button>
          <el-button class="sureToggle" type="primary" @click="sureSwitch">确 定</el-button>
        </div>
    </el-dialog>
...
import {
  ...
  addTypeProperty,			//新增属性接口
  addTypeAbility,				//新增能力接口
  addTypeEvent,					//新增事件接口
  ...
  methods: {
    ...
    UserDefinedFunctionPost() {
      if(this.activeName == 0){
        ...
        this.filterInput()
      	let select = false;
      	if(this.addUserSetting.openFilter,this.addUserSetting.filterInput) {
        ...
        this.$refs['addUserSetting'].validate(valid => {
          if (valid && select) {
            ...
            addTypeProperty(settingParams).then(res => {
              if(res.err_CODE == 0){
        ...
        this.$refs['addAbilityForm'].validate(valid => {
          if (valid) {
						...
						addTypeAbility(abilityParams).then(res => {
              if(res.err_CODE == 0){
                this.$message.success(res.err_MESSAGE);
                this.$emit('dialogClose', true);
              }else{
        ...
        this.$refs['addEventForm'].validate(valid => {
          if (valid && select) {
            ...
            addTypeEvent(eventParams).then(res => {
              if(res.err_CODE == 0){
                this.$message.success(res.err_MESSAGE);
                this.$emit('dialogClose', true);
              }else{
    ...
    validateMaxOrMin(rule,value,callback) {  // 验证最大值和最小值
      ...
      if(dataType == 1){
        ...
        const reg = /^(([-]?[0-9][0-9]{0,9})|(([-]?[0]\.\d{0,5}|[-]?[1-9][0-9]{0,9}\.\d{1,5})))$/ ;
        if(reg.test(min) && !isNaN(min)){      
            min = Number(min);
            if(reg.test(max) && !isNaN(max)){
                max = Number(max);
